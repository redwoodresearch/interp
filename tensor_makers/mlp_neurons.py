...
def get_lvnt(model, params, tokenizer, string):
    ...
    def fn():
        log = inference(model, params, token_ids, to_log_idxed=[KeyIdxs(f"blocks.mlp.linear1", Idxs.all())])
        # Let's visualize the attention scores by layer and head
        neurons = jnp.squeeze(log[f"blocks.mlp.linear1"], axis=1)  # shape = (layer, head, seq)
        return neurons
